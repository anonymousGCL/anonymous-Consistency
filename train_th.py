...
from gcc.datasets.data_util import batcher, labeled_batcher, continual_batcher
...
def parse_option():
    ...
    parser.add_argument("--dgl_file", type=str, default="/data/")
    ...
    # dataset definition
    parser.add_argument("--dataset", type=str, default="h-index",
                        choices=["dgl", "wikipedia", "blogcatalog", "usa_airport", "brazil_airport", "europe_airport",
                                 ...
                                 "h-index-rand-1", "h-index-top-1", "h-index", "cora", "pubmed", "texas", "DD242",
                                 "cornell", "wisconsin",
                                 "DD687", "DD68", "citeseer"] + GRAPH_CLASSIFICATION_DSETS)
    ...
    parser.add_argument("--reg-coeff", type=int, default=10)
    ...
    # specify folder
    parser.add_argument("--model-path", type=str, default='saved1', help="path to save model")
...
def option_update(opt):
    opt.model_name = "{}_moco_{}_{}_{}_layer_{}_lr_{}_decay_{}_bsz_{}_hid_{}_samples_{}_nce_t_{}_nce_k_{}_rw_hops_{}_restart_prob_{}_aug_{}_ft_{}_deg_{}_pos_{}_momentum_{}_ct_{}".format(
        ...
        opt.alpha, opt.continual,
    )
    if args.finetune:
        opt.model_name = "path_ftgcl" + str(args.reg_coeff) + "_" + args.dataset
    elif args.continual:
        opt.model_name = "path_ctgcl" + str(args.reg_coeff) + "_" + args.dataset
    else:
        opt.model_name = "path"
...
def square_loss(feature_a, feature_b, label):
    ...
    for j in range(len(mat)):
        tmp = torch.mean(feature_mix[mat[j]], dim=0)
...
def continual_training(
    ...
    for idx, batch in enumerate(train_loader):
        ...
        # ===================backward=====================
        optimizer.zero_grad()
        term = square_loss(feat_q, feat_k, torch.Tensor(label_q))
        print(term)
...
def main(args):
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            pretrain_args.continual = args.continual
    ...
    # train_dataset1 = torch.utils.data.Subset(dataset1, train_idx)
    # valid_dataset1 = torch.utils.data.Subset(dataset1, test_idx)
    ...
    def fn():
        if args.finetune:
            return labeled_batcher()
        elif args.continual:
            return continual_batcher()
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        collate_fn=fn(),
        shuffle=True if args.finetune or args.continual else False,
        num_workers=args.num_workers,
        worker_init_fn=None
    ...
    if args.continual:
        criterion = modifiedNCESoftmaxLoss()
